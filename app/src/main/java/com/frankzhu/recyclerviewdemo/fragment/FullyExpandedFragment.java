...
public class FullyExpandedFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (type == TYPE_GRID_LAYOUT) {
            mRecyclerView.setLayoutManager(new FullyGridLayoutManager(getActivity(), 2));//这里用线性宫格显示 类似于grid view
        } else if (type == TYPE_STAGGERED_GRID_LAYOUT) {
            mRecyclerView.setLayoutManager(new StaggeredGridLayoutManager(2, OrientationHelper.VERTICAL));//这里用线性宫格显示 类似于瀑布流
        } else {
            mRecyclerView.setLayoutManager(new FullyLinearLayoutManager(getActivity()));//这里用线性显示 类似于list view
        }
